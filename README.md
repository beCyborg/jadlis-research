...
`/jadlis-research:research-settings` — таблица всех источников: провайдеры (Grok, Codex), 14 каналов и
слои с ключами, у каждого настройка, доступ, когда проверялся и что теряется без него. Там же источник
выключается насовсем.

Два состояния. `auto` — доступ проверяется перед прогоном (ключ, бинарник, баланс), результат кэшируется.
`off` — источник не используется и не проверяется: ни вызова, ни секунды на пробу.

Выключил Grok — канал `grokweb` больше не предлагается, а `twitter` идёт по ключевым словам через
TwitterAPI.io с фолбэком на Brave `site:x.com`.

Настройка лежит в `~/.claude/plugins/data/jadlis-research-jadlis/source-settings.json` (рядом
`probe-cache.json` с результатами проб) и переживает обновление плагина, но не удаление без `--keep-data`.
Ключи заводятся отдельно — командой `/jadlis-search:keys`.

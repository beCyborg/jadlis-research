...
check "version=0.9.0" test "$(jq -r .version "$PJ")" = "0.9.0"
...
echo "=== Section 8: Packaging Validation ==="
MJ="$PLUGIN_ROOT/.claude-plugin/marketplace.json"
check "marketplace.json exists" test -f "$MJ"
check "marketplace.json valid JSON" jq . "$MJ"
check "marketplace plugins[0].source=./" test "$(jq -r '.plugins[0].source' "$MJ")" = "./"
check "marketplace plugins[0].name=jadlis-research" test "$(jq -r '.plugins[0].name' "$MJ")" = "jadlis-research"
check_not ".mcp.json not tracked" test -n "$(git -C "$PLUGIN_ROOT" ls-files .mcp.json)"
check ".mcp.json.example tracked" test -n "$(git -C "$PLUGIN_ROOT" ls-files .mcp.json.example)"
check "setup skill exists" test -f "$PLUGIN_ROOT/skills/setup/SKILL.md"
check "session-init executable" test -x "$PLUGIN_ROOT/scripts/session-init.sh"

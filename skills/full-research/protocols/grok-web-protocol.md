...
grok-4.5: exit 0, живой URL, ~20 сек на вызов). MCP `mcp__grok-mcp__web_search` НЕ использовать —
...
Парсинг: `... | jq -r '.text'` — ответ агента внутри `.text` (JSON-обёртка CLI: `{text, stopReason, sessionId, requestId, thought}`).

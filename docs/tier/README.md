...
1. **Разведка + интервью** — обязательный вопрос: какое решение примешь.
2. **Каналы параллельно** — по умолчанию 7 из 14: web ×3, Reddit, X, HN, Substack.
3. **Гейт достаточности** — ответили 2+ канала и 2+ семьи источников. Нет → `insufficient-sources`, в vault не пишется ничего.
4. **Куратор** — до 16 утверждений с evidence-префиксами.
5. **urlhealth + снапшот-гейт** — потолок MEDIUM на слабой цитате.
6. **Два верификатора на утверждение, по одному от каждой семьи** — первый оспаривает утверждение там, где оно найдено (с Reddit → на Reddit, из веба → через Brave), второй берёт другую семью. Веб и сообщества разошлись → «расхождение семей»: в отчёт попадают обе версии, приоритет по типу утверждения (факт → веб, опыт → сообщества). Одиночное исключение → третий голос Codex (не больше 8 эскалаций за прогон).
7. **Синтез** — только выжившее; отсеянное = строка с причиной.
8. **Заметка в `Знания/Ресерчи/`** — `verified: false`, пока ты её не прочитал.
...
Так работает фаза проверки: одна линза ищет опровержение в той же семье, где утверждение найдено, вторая берёт источник другого типа. Расхождение веба и сообществ не усредняется и не добивается третьим веб-голосом — читатель видит обе версии и кому приоритет.
...
### Проверенные факты
- Поиск по статике решается внешним индексом, а не генератором (2 голоса: веб + сообщества) [w2·B2]
- Порог боли — редакторы без git, а не объём страниц (2 голоса: веб + сообщества) [r5·C2]
- Картинки и вложения — главный источник ручной работы при переносе (1 голос — только сообщества) [hn3·B2]

### Веб и сообщества расходятся
- «Инкрементальная сборка держит 10 000 страниц без замедления» — Веб: страница вендора и бенчмарк [w4·B3]. Сообщества: три треда о сборке по 20 минут на реальных вики [r7·C3]. Приоритет: сообщества — утверждение опытное.

### Спорные факты
- «Переезд занимает один вечер» — единственный голос против, третий голос исключение не подтвердил

> [!note]- Методология и проверка
> Каналы: 5, семей источников: 4. Проверено 11 утверждений (по голосу от веба и от сообществ):
> 6 подтверждено (1 одним голосом), 1 расхождение веб/сообщества, 1 спорное, 3 отсеяно
> (2 оспорено, 1 устарело), эскалаций в Codex: 2.
...
Получишь: сначала интервью (первый вопрос всегда «какое решение ты примешь по итогам»), затем один вопрос-подтверждение с формулировкой и каналами — ответ «Запустить» и есть гейт запуска. На выходе заметка в `Знания/Ресерчи/` с тремя корзинами (проверенные / спорные / отсеянные) и сводка: какие каналы отработали, сколько утверждений подтвердилось, что отсеяно и почему.

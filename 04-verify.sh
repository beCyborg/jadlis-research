...
# -----------------------------------------------
echo ""
echo "--- 10. Twitter SKILL.md (Grok-MCP) ---"
...
grep -q "^user-invocable: false" "$SKILL" && pass "user-invocable: false" || fail "user-invocable not set"
! grep -q "disable-model-invocation" "$SKILL" && pass "no disable-model-invocation" || fail "disable-model-invocation present"
grep -q "x_search" "$SKILL" && pass "x_search present" || fail "x_search missing"
grep -q "mcp__plugin_jadlis-research_twitter__x_search" "$SKILL" && pass "x_search namespace present" || fail "x_search namespace missing"
grep -q "XAI_API_KEY" "$SKILL" && pass "XAI_API_KEY documented" || fail "XAI_API_KEY missing"
grep -q "console.x.ai" "$SKILL" && pass "console.x.ai referenced" || fail "console.x.ai missing"
grep -qi "DD-MM-YYYY\|DD.MM.YYYY" "$SKILL" && pass "date format DD-MM-YYYY documented" || fail "date format missing"
grep -q "include_inline_citations" "$SKILL" && pass "inline citations documented" || fail "inline citations missing"
grep -q "allowed_x_handles" "$SKILL" && pass "handle filtering documented" || fail "handle filtering missing"
grep -qi "narrative synthesis\|agentic" "$SKILL" && pass "agentic nature documented" || fail "agentic nature missing"
grep -qi "category.*tweet\|category: tweet" "$SKILL" && pass "category:tweet fallback documented" || fail "category:tweet fallback missing"
grep -qi "no.*filter\|prohibit\|only.*category\|crash\|500" "$SKILL" && pass "Exa constraint documented" || fail "Exa constraint missing"
! grep -q "OPENTWITTER_API_KEY" "$SKILL" && pass "no old OPENTWITTER_API_KEY" || fail "old OPENTWITTER_API_KEY still present"
! grep -q "6551" "$SKILL" && pass "no old 6551 proxy reference" || fail "old 6551 proxy reference still present"

# -----------------------------------------------
echo ""
echo "--- 11. Twitter parameters.md (Grok-MCP) ---"
...
grep -q "x_search" "$REF" && pass "x_search in ref" || fail "x_search missing from ref"
grep -q "XAI_API_KEY" "$REF" && pass "XAI_API_KEY in ref" || fail "XAI_API_KEY missing from ref"
grep -q "mcp__plugin_jadlis-research_twitter__x_search" "$REF" && pass "x_search namespace in ref" || fail "x_search namespace missing from ref"
grep -qi "DD-MM-YYYY" "$REF" && pass "date format in ref" || fail "date format missing from ref"
...
grep -q "merterbak\|Grok-MCP\|grok-mcp" "$REF" && pass "Grok-MCP package in ref" || fail "Grok-MCP package missing from ref"
! grep -q "OPENTWITTER_API_KEY" "$REF" && pass "no old OPENTWITTER_API_KEY in ref" || fail "old OPENTWITTER_API_KEY still in ref"
